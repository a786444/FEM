function [ndime,ntrac,mate,trac_force,trac_coor,nelnd]=ReadData(filename)
...
ind=find((strcmp(data{1,1},'trac_coor:'))==true);
trac_coor=zeros(ndime,ntrac);
for i=1:ndime*ntrac
    ind=ind+1;
    trac_coor(i)=str2double(data{1}{ind});
end
% trac_nodes=zeros(ndime,ntrac);
% for i=1:ntrac
%     trac_nodes(i)=find(p(1,:)==trac_coor(1,i) & p(2,:)==trac_coor(2,i));
% end
trac_force=zeros(ndime,ntrac);
